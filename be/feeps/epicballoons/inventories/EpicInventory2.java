...
import be.feeps.epicballoons.balloons.EpicHeads2;
...
public class EpicInventory2 {
    public static Map<Player, EpicInventory2> epicInventoryMap = new HashMap<>();
    private Inventory inventory;
    private Player player;

    public EpicInventory2(Player player){
        this.player = player;
        this.inventory = Bukkit.createInventory(null, 45, LangConfig.Msg.InventoryNameTwo.toString());
        epicInventoryMap.put(this.player, this);
    }

    private void create(){
        for(EpicHeads2 heads : EpicHeads2.values()){
        ...
        this.setItem(new ItemStack(ItemsUtils.getCustomSkull("ZjdhYWNhZDE5M2UyMjI2OTcxZWQ5NTMwMmRiYTQzMzQzOGJlNDY0NGZiYWI1ZWJmODE4MDU0MDYxNjY3ZmJlMiJ9fX0=")), 36, LangConfig.Msg.InventoryItemBackMenu.toString(), Collections.singletonList(LangConfig.Msg.InventoryBackDesc.toString()));
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if (item.getItemMeta().getLore().get(0).equalsIgnoreCase(LangConfig.Msg.InventoryLoreHasPermission.toString())) {
            ...
            if (item.getItemMeta().getLore().get(0).equalsIgnoreCase(LangConfig.Msg.InventoryLoreNoPermission.toString())) {
                ...
                if (item.getItemMeta().getLore().get(0).equalsIgnoreCase(LangConfig.Msg.InventoryBackDesc.toString())) {
                    this.player.closeInventory();
                    new EpicInventory(player).openInv();
                } else {
